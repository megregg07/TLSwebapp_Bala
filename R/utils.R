########################################
## 'utils' file that accompanies Bala's 
## version of the TLS 'Part II' Webapp
##
## 12/01/2025
########################################
...
TLS_getTandP <- function(dat) {
  ...
  nT <- nrow(dat)
  ## detrmine the number of TLS positions
  nP <- ncol(dat)/3
  return(list(nTargets = nT, nPositions = nP))
}

## function that converts Zc files to mm, if units are not mm
Zc_to_mm <- function(Zc_input, units = c("mm", "meters", "inches", "feet")) {
  ## determine the units
  ...
  Zc <- switch(units, 
               mm = Zc_input, 
               meters = Zc_input*1000, 
               inches = Zc_input*25.4, 
               feet = Zc_input*304.8)
  return(list(Zc = Zc))
}
...
## 
create_Zc <- function(f1, f2, f3, f4, units = c("mm", "meters", "inches", "feet")){
  ## determine the units
  units <- match.arg(units)
  ## ensure the files have the target ID column as "Target"
  colnames(f1) <- colnames(f2) <- colnames(f3) <- colnames(f4) <- c("Target", "X", "Y", "Z")
  
  data_cbind <- merge(merge(merge(f1, f2, by = "Target", suffixes = c('.p1', '.p2')), 
                            f3, by = "Target"), 
                      f4, by = "Target", suffixes = c('.p3', '.p4'))
...
lengthErrors <- function(Dtape, Zc, Zc_names){
  ...
  if(sum((1-1*(Dtape[,2] %in% Zc_names)) + (1-1*(Dtape[,3] %in% Zc_names)))!=0){
    stop(print("Target names in Tape Measurement data need to match those in TLS Test Data"))
  } else{
    ...
    for(l in 1:length(Dtape[,1])){
      ...
      statement_l <- data.frame(
        ...
        statement = paste0("Length ", row_l[,1], " is defined by ", Zc_names[targets_l[1]], " and ", 
                           Zc_names[targets_l[2]], ". The reported tape measurement of Length ", 
...
##
## Function to get composite coordinates
## Input: Zc matrix (dimension nT x nP*3)
## Output: composite coordinates (dimension: nT x 3), in the Position 1 frame of reference
##
## Made this function from the revised 'Zc_to_R' function
##
get_composite <- function(Zc) {
  ## determine the number of positions
  nP <- ncol(Zc)/3
  ## determine the number of targets
  nT <- nrow(Zc)
...
TLS_cov_check <- function(dmat1, dmat2, conf.level = 0.95) {
  ...
  RESULT <- ifelse(pval > alpha, 'consistent', 'INCONSISTENT')
  conclusion <- switch(RESULT,
                       consistent = 'The instrument state has not changed.', 
                       INCONSISTENT = 'The instrument state HAS CHANGED.'
  )
  interpretation <- paste("At a", 1-conf.level, "significance threshold (correponding to a",  
                          conf.level, "confidence level), there is", 
                          ifelse(RESULT=="consistent", 'insufficient', 'sufficient'), 
  "evidence to conclude that the instrument's precision significantly differs between the 
  baseline data and the test data.")
...
##
partIerrorplot <- function(dat, threshold= NA) {
  errorplot <- ggplot(dat, aes(x = Length, y = Error, color = as.factor(ID))) +
    geom_point() +
